public static class DateSelection
{
    ...
    {
        int month = monthMenu();
        MenuNavigation dayMenu = new MenuNavigation(DaysInSelectedMonth(month), Months[month - 1]);

        int year = 2026;
        return DateSelection.GetDateFromCoordinate(dayMenu.Start2D(), year, month);
    }
}